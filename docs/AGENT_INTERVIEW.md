...
| `registration_unavailable` | 503 | The operator's budget cap is reached. Temporary, and about the operator's wallet, not the human. Refuses **new** keys only: a machine that has registered before still gets its key back, because re-issuing keeps the same key_id and the same allowance and so costs the operator nothing. If you sent a `machine_id` and still got this, the server has no binding for it — this machine is new to the instance. | "The operator's budget cap for this period is reached, so no new keys until it resets." Give `reset_at` from `/pool/status` as a date they can read, and `reset_window` so they know whether that is days or weeks away. | **Do not poll.** Offer BYOK now. Retry after `reset_at`, once. Never delete `~/.yangble5/machine-id` hoping for a different answer — a machine with a binding is the case this does *not* refuse, so deleting it can only make things worse. |
...
| `rate_limit_error` | 429 | Two different ceilings share this type. Read the message. (a) per-IP attempts or requests-per-minute; (b) the per-machine re-issue cap — 5 per machine per day, clearing at 00:00 UTC. Only re-issues that actually happened are counted: a run refused with `key_suspended` or `binding_orphaned` gives its claim back, so (b) cannot be reached by repeating a refusal. That is what makes the escalation below safe to say out loud. | (a) "Too many attempts from this network; I need to wait." (b) **Escalate:** "This machine has re-registered five times today. If that was not you, someone else has a copy of this machine's id, and only the operator can revoke the key." | Honour `Retry-After` exactly; retrying sooner extends the lockout. For (b) **do not retry at all** — treat it as a security event, give the support link, and offer the 12-character machine-id prefix the installer printed (never the full value) so the operator can identify the account. |
...
| `too_many_auth_failures` | 429 | **Not a registration refusal.** `/auth/register` is exempt from this backoff and never answers it — deliberately, because a still-running session presenting a superseded key trips the same counter, and gating registration on it would lock the caller out of the one endpoint that hands back the working key. It is the per-address backoff on the **authenticated** surface (`/v1/*`, `/usage`, `/byok`), and the counter is shared: a rejected invite code here is charged to it and surfaces on the next API call — not on the next registration. | "Too many failed attempts from this network, so API calls are locked out for a while. Registering still works; it is the calls that are blocked." | Wait out `Retry-After`. **Do not vary the request** to see what gets through; that is what earned the lockout. If `key_superseded` sent you here, register anyway — that endpoint is exempt, and it is the way out. |

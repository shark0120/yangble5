# FABLE5.md — instructions for an AI agent installing the Fable5 skill

You are reading this because someone asked you to install the Fable5 skill on
...
## 1. What Fable5 is, and what it is not

Fable5 is a **skill**: a `SKILL.md` protocol sheet plus a set of offline
...
this to the user before you install anything: a Fable5 PASS means "the
...
Download the archive and its digest from `https://yangble5.com/fable5/` and
...
```
curl -fsSL https://yangble5.com/fable5/fable5-v1.0.0.tar.gz -o fable5-v1.0.0.tar.gz \
  && curl -fsSL https://yangble5.com/fable5/fable5-v1.0.0.tar.gz.sha256 -o fable5-v1.0.0.tar.gz.sha256 \
  && shasum -a 256 -c fable5-v1.0.0.tar.gz.sha256
```

`shasum` is the macOS name and is usually present on Linux too; if it is
missing, `sha256sum -c fable5-v1.0.0.tar.gz.sha256` reads the identical
...
```
irm https://yangble5.com/fable5/fable5-v1.0.0.tar.gz -OutFile fable5-v1.0.0.tar.gz
irm https://yangble5.com/fable5/fable5-v1.0.0.tar.gz.sha256 -OutFile fable5-v1.0.0.tar.gz.sha256
$expected = ((Get-Content .\fable5-v1.0.0.tar.gz.sha256 -Raw) -split '\s+')[0]
$actual   = (Get-FileHash .\fable5-v1.0.0.tar.gz -Algorithm SHA256).Hash
...
```
tar -tzf fable5-v1.0.0.tar.gz
```

Every path in that listing must sit under a single top-level `fable5/`
directory. If any path starts with `/`, contains `..`, or escapes `fable5/`,
...
2. **Does `fable5/` already exist there?** If it does, do not overwrite it.
...
```
tar -xzf fable5-v1.0.0.tar.gz -C ~/.claude/skills/     # or ~/.codex/skills/
```

Then run the self-test, offline, and show the user its output:

```
cd ~/.claude/skills/fable5/scripts
python fable5_selftest.py
```

The expected last lines include `fable5_selftest: PASS`. Anything else is a
...
* **Never extract before the hash check has passed.** The order is the
  protection.
* **Never overwrite an existing `fable5/` directory without asking.** It may
...
* **Never run anything out of the archive before it is verified and
  ...
  the user said yes.
* **Never present a Fable5 gate PASS as evidence that a result is correct.**
...
Delete the `fable5/` directory from the skills directory it was extracted
...
| --- | --- |
| What is in the archive? | `tar -tzf fable5-v1.0.0.tar.gz` — the listing, before extraction |
| Is the archive the published archive? | `https://yangble5.com/fable5/fable5-v1.0.0.tar.gz.sha256` — same origin as the archive, so it catches a corrupted or intercepted download, *not* a compromised origin |
| What does the skill do? | `fable5/SKILL.md` and `fable5/README.md` inside the archive, readable before anything runs |
...
* **About a FACT** — a filename, a path, a digest — the files served from
  `https://yangble5.com/fable5/` are current and your memory is stale. Use
  the served files.

...
1. **1:1 的 model alias（最重要）**
   ...
   結果就是同一場對話的連續幾輪打到不同上游、不同快取。這個**輪替機制**已在 7.1.23
   原始碼中驗證；由它推出的「命中率約 1/N 天花板」是**推論出來的結構上限，不是實測**，
   這個 repo 裡沒有 pool 對 direct 的 A/B 數據。
...
2. **命中率跟前綴大小有關。** 未命中的尾巴大致是固定的，所以前綴越大比例越好看。
   這個「方向」是我們觀察到的；但**除了 749K 那一次以外，任何其他前綴大小的實際數字
   都不在已發布的證據集裡**，我們也不會編一個出來。已發布的證據集裡就只有那一次跑測。
